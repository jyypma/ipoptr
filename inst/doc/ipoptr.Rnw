...
Installing the \texttt{ipoptr} package is not as straightforward as most other R packages, because it depends on Ipopt, which has to be installed separately. Ipopt and the R interface to Ipopt are distributed separately because of differences in licenses between Ipopt, some of the libraries it depends on, and R. There are ways to simplify the Ipopt part of the installation by using some of the pre-compiled libraries of Ipopt with Mumps\footnote{Any help to simplify the installation process is appreciated and if someone uses the pre-compiled libraries and has a \texttt{Makefile}, please let me know.}. To install (and compile) Ipopt and the R interface a C/C++ compiler has to be available. Detailed installation instructions are available on \texttt{http://www.coin-or.org/Ipopt/documentation}. On Windows I was succesful using MSYS, on Ubuntu no additional tools were needed. 

In the description of the installation of the R interface, I will assume that you have a working installation of Ipopt. You need to be able to compile R packages from source. Since Ipopt was already successfully installed, compilers are probably available. On Windows platforms Rtools is needed to compile R packages from source\footnote{\texttt{http://www.murdoch-sutherland.com/Rtools/}}. 

First, a file \texttt{Makevars} in the source directory of the interface package needs to be configured for your system from the file \texttt{Makevars.in}. The \texttt{configure\_ipoptr.R} script should make this easier. Start R, go to the directory where the \texttt{ipoptr} interface is located and load the script. In the code below, you should change the directories to the correct ones for your system.
<<configureIpoptrStep1, eval=FALSE>>=
setwd('~/ipoptr')
source('configure_ipoptr.R')
@
Running this script defines a function \texttt{configure\_ipoptr}. Run the \texttt{configure\_ipoptr} command with the directory of the Ipopt build and the directory where \texttt{ipoptr} is located as arguments, e.g.
<<configureIpoptrStep2, eval=FALSE>>=
configure_ipoptr( ipopt.build.dir = "/cvos/shared/apps/Ipopt/3.5.5-source",
                  ipoptr.dir = "~/ipoptr" )
@
The \texttt{ipoptr.build.dir} should have a file called \texttt{Makefile} in the subdirectory \texttt{/Ipopt/examples/hs071\_cpp}. If there were no errors, then a file called \texttt{Makevars} (or \texttt{Makevars.win} if you're using Windows) has now been created in the directory \texttt{ipoptr/src}. 

You can then install the package with the command
<<installIpoptPackage, eval=FALSE>>=
install.packages('~/ipoptr', repos=NULL, type='source')
@
where the first argument specifies the directory where the R interface to Ipopt is located. You should now be able to load the R interface to Ipopt and read the help.
